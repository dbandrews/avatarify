...
def predict(
    driving_frame, source_image, relative, adapt_movement_scale, fa, device="cuda"
):
    global start_frame
    global start_frame_kp
    ...
    with torch.no_grad():
        ...
        driving = (
            torch.tensor(driving_frame[np.newaxis].astype(np.float32))
            .permute(0, 3, 1, 2)
            .to(device)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-pad", dest="no_pad", action="store_true", help="don't pad output image"
    )
    ...
    parser.add_argument(
        "--no-stream", action="store_true", help="On Linux, force no streaming"
    )
    ...
    parser.add_argument(
        "--avatars", default="./avatars", help="path to avatars directory"
    )
    ...
    if _streaming:
        ret, frame = cap.read()
        stream = pyfakewebcam.FakeWebcam(
            f"/dev/video{opt.virt_cam}", frame.shape[1], frame.shape[0]
        )
    ...
    while True:
        ...
        if passthrough:
            ...
            pred = predict(
                frame, avatar, opt.relative, opt.adapt_scale, fa, device=device
            )
            out = pred
        ...
        preview_frame = cv2.addWeighted(
            avatars[cur_ava][:, :, ::-1], overlay_alpha, frame, 1.0 - overlay_alpha, 0.0
        )
        ...
        if find_keyframe:
            preview_frame = cv2.putText(
                preview_frame, display_string, (10, 220), 0, 0.5, (255, 255, 255), 1
            )

        if show_fps:
            fps_string = f"FPS: {fps:.2f}"
            preview_frame = cv2.putText(
                preview_frame, fps_string, (10, 240), 0, 0.5, (255, 255, 255), 1
            )
